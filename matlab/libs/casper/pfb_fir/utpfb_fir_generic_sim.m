...
function[dout, result] = utpfb_fir_generic_sim(varargin)
dout = []; result = -1;
log_group = 'utpfb_fir_generic_sim_debug';

utlog('entering utpfb_fir_sim', {'trace', log_group});
...
if ~isempty(find(results ~= 0)),
    utlog('error getting parameters from varargin',{'error', 'utfft_sim_debug'});
    return;
end
...
[r,c] = size(din);
if c~=1,
    utlog('din must be single column',{'error', 'utfft_sim_debug'});
    return;
end
...
if strcmp(type, 'pfb_fir_generic'),
    if strcmp(version, '0'),
    ...
    utlog(['unknown type ',type], {'error', log_group});
end
...
result = 0; 
utlog('exiting utpfb_fir_generic_sim', {'trace', log_group});
